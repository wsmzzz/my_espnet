...
class FeedForwardTransformer(TTSInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args=None):
        ...
        print(get_parameter_number(self.duration_predictor))
    ...
    def _forward(
        ...
        if is_inference:
            ...
            if ds is not None:
                ...
                hs = self.length_regulator(hs, d_outs)  # (B, Lmax, adim)
        else:
            ...
            if ds is None:
                with torch.no_grad():
                    ...
                    max_len= torch.max(torch.sum(ds,dim=-1))
                d_outs = self.duration_predictor(hs, d_masks)  # (B, Tmax)
                hs = self.length_regulator(hs, ds, max_len)  # (B, Lmax, adim)
            else :
                max_len=None
                d_outs = self.duration_predictor(hs, d_masks)  # (B, Tmax)
                hs = self.length_regulator(hs, ds, max_len)  # (B, Lmax, adim)
    ...
    def forward(self, xs, ilens, ys, olens, spembs=None, extras=None, *args, **kwargs):
        ...
        if self.teacher is None :
            ds=xs[:,2,:].view(xs.shape[0],xs.shape[-1])
        # forward propagation
    ...
    def inference(self, x, inference_args, spemb=None, *args, **kwargs):
        ...
        ilens = torch.tensor([x.shape[1]], dtype=torch.long, device=x.device)
        xs = x.unsqueeze(0)
        if xs.shape[1]==3:
            ds=xs[:,2,:]
            ds=ds[:,:-1]
        ...
        # inference
        _, outs, ds_outs = self._forward(
            xs, ilens, spembs=spembs,ds=ds, is_inference=is_inference, alpha=alpha,
        )  # (1, L, odim)

        return {'mel_feats':outs[0],
                'dur':ds_outs.squeeze().detach().cpu().numpy(),
