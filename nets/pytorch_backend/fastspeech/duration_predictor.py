...
class Duration_variance_Predictor(torch.nn.Module):
    ...
    def _forward(self, xs, x_masks=None, is_inference=False):
        ...
        variance = xs[:,:,1].squeeze(-1)
        variance = torch.clamp(
               variance.exp(), min=2,max=25
            ).float()

        if is_inference:
            # NOTE: calculate in linear domain
            dur = torch.clamp(
                torch.round(dur.exp() - self.offset), min=0
            ).long()  # avoid negative value

        if x_masks is not None:
            dur = dur.masked_fill(x_masks, 0.0)
            variance = variance.masked_fill(x_masks, 1)
...
def batch_one_hot(input,num_class):
    L=input.shape[1]
    onehots=[]
    for i in input:

        y_one_hot = torch.zeros(L, num_class).scatter_(1, i.unsqueeze(-1), 1)
        onehots.append(y_one_hot)
    onehots=torch.stack(onehots,dim=0)
    return onehots
