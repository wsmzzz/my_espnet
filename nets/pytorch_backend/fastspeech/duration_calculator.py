...
from espnet.nets.pytorch_backend.rhythm_tts_tacotron2 import Tacotron2
from espnet.nets.pytorch_backend.rhythm_tts_transformer import Transformer
...
class DurationCalculator(torch.nn.Module):
    ...
    def forward(self, xs, ilens, ys, olens, spembs=None):
        ...
        if isinstance(self.teacher_model, Transformer):
            ...
            xs=xs[:,0,:].view([xs.shape[0],xs.shape[-1]])
