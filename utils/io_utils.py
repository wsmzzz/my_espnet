...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        pad_mask_dict=OrderedDict()
        phone_mask_dict=OrderedDict()
        uttid_list = []  # List[str]
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output"]):
                    ...
                    if 'dur' in inp:
                        x_1 = np.reshape(np.fromiter(
                            map(int, inp["tokenid"].split()), dtype=np.int64
                        ),[1,-1])
                        x_2=np.reshape(np.fromiter(
                            map(int, inp["ryhthm_id"].split()), dtype=np.int64
                        ),[1,-1])
                        x_3=np.reshape(np.fromiter(
                            map(int, inp["dur"].split()), dtype=np.int64
                        ),[1,-1])
                        x=np.concatenate([x_1,x_2,x_3],axis=0)
                        
                        if self.load_mask:
                            file=h5py.File(self.mask_dir,'r')
                            pad_mask=file['pad_group'][uttid].value
                            phone_mask=file['phone_group'][uttid].value
                            pad_mask_dict.setdefault(inp['name'],[]).append(pad_mask)
                            phone_mask_dict.setdefault(inp['name'],[]).append(phone_mask)
                            file.close()
        ...
        if self.mode == "asr":
            ...
            if self.load_mask :
                return_batch, uttid_list = self._create_batch_tts(
                    x_feats_dict, y_feats_dict, uttid_list, eos,pad_mask_dict,phone_mask_dict
                )
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos,pad_mask_dict=None,phone_mask_dict=None):
        """Create a OrderedDict for the mini-batch
        ...
        if self.sort_in_input_length:
            # sort in input lengths
            if len(xs[0].shape)==2:
                nonzero_sorted_idx = sorted(nonzero_idx, key=lambda i: -len(xs[i][0,:]))
        ...
        if len(xs[0].shape)==2:
           ...
           xs = [np.append(x, eos,-1) for x in xs]
           
        else:
         
           xs = [np.append(x, eos) for x in xs]
         
        if self.load_mask and pad_mask_dict is not None:
            pad_masks=list(pad_mask_dict.values())[0]
            phone_masks=list(phone_mask_dict.values())[0]
            pad_masks=[pad_masks[i] for i in nonzero_sorted_idx]
            phone_masks=[phone_masks[i] for i in nonzero_sorted_idx]
        ...
        if self.load_input:
            ...
            if self.load_mask:
                return_batch = OrderedDict(
                    [(x_name, xs), (y_name, ys),(spembs_name, spembs), (spcs_name, spcs),('pad_mask',pad_masks),('phone_mask',phone_masks),]
                )
